//
// Created by awalol on 2026/5/15.  (ESP-IDF port — unchanged from the Pico build)
//
// Output-state manager: holds the current DualSense set-state (LED, rumble,
// trigger FFB, etc.) and merges host SET_REPORT 0x02 updates into it gated by
// the report's "Allow*" flag bits. state_set() copies the current state into an
// outgoing 0x31/0x32 report body.
//
...
void state_update(const uint8_t *data, const uint8_t size) {
    ...
    (void) kAudioControlOffset;
    (void) kMuteControlOffset;
    (void) kAudioControl2Offset;
    (void) kHapticLowPassFilterOffset;
}
